...
export function FeedbackMessage({ icon, titulo, descricao, buttonText, redirectTo, type }: FeedbackMessageProps) {
  ...
  const hoverBgColor = type === "error" ? "hover:bg-red-700" : "hover:bg-green-700";

  return (
    <>
      <div className={`flex self-center ${iconColor}`}>{icon}</div>
      <div className="flex flex-col gap-1">
        <h1 className={`text-2xl text-center ${titleColor} font-bold`}>{titulo}</h1>
        <p className="text-center text-md text-gray-500">{descricao}</p>
      </div>
      <Button
        size="lg"
        className={`${buttonBgColor} ${hoverBgColor} text-white font-medium`}
