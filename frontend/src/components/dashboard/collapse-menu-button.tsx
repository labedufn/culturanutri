  ...
  return isOpen ? (
    <Collapsible open={isCollapsed} onOpenChange={setIsCollapsed} className="w-full">
      <CollapsibleTrigger className="[&[data-state=open]>div>div>svg]:rotate-180 mb-1" asChild>
        <Button variant={active ? "secondary" : "ghost"} className="w-full justify-start h-10 bg-zinc-800">
    ...
    <DropdownMenu>
      <TooltipProvider disableHoverableContent>
        <Tooltip delayDuration={100}>
          ...
          <TooltipContent side="right" align="start" alignOffset={2} className="text-white bg-zinc-600">
            {label}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
