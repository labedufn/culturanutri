...
type AvaliacaoGestoresConhecimentoProps = {
...
export function AvaliacaoGestoresConhecimento({ onFormValidation }: AvaliacaoGestoresConhecimentoProps) {
  ...
  useEffect(() => {
    const fetchUserId = async () => {
      ...
      if (id) {
        ...
        if (storedUserId === id) {
          const storedData = localStorage.getItem("conhecimentoGestor");
          if (storedData) {
  ...
  useEffect(() => {
    if (userId) {
      ...
      localStorage.setItem("conhecimentoGestor", JSON.stringify(data));
