...
export function redefinirSenhaToken(middleware: CustomMiddleware) {
  return async (request: NextRequest, event: NextFetchEvent, response: NextResponse) => {
    const token = request.nextUrl.searchParams.get("token");
    const currentUrl = request.nextUrl.pathname;

    if (token === null && currentUrl === "/autenticacao/redefinir-senha") {
...
export const config = {
  matcher: ["/autenticacao/redefinir-senha"],
};
